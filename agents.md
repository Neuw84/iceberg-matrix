...
│   │   ├── compatibility-data.json   # Aggregated data (platforms + features + support)
│   │   ├── aws.json                  # AWS platform support entries
│   │   ├── azure.json                # Azure platform support entries
│   │   ├── databricks.json           # Databricks support entries
│   │   ├── gcp.json                  # GCP support entries
│   │   ├── oss.json                  # Open-source engines support entries
│   │   ├── snowflake.json            # Snowflake support entries
...
3. Add the same platform and support entries to `src/data/compatibility-data.json` (this is the aggregated file the app reads at runtime).
...
2. Add the feature to the `"features"` array in `src/data/compatibility-data.json`.
...
- The `compatibility-data.json` file is the single source of truth at runtime. Vendor-specific files (`aws.json`, `gcp.json`, etc.) exist for organizational clarity but must be kept in sync with the aggregated file.
